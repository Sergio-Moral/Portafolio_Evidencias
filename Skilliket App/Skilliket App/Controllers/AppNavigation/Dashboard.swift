...
class Dashboard: UITableViewController, UITabBarDelegate {
    ...
    func setupTitleWithIcon() {
        ...
        attachment.image = UIImage(systemName: "circle.dotted.circle")?.withTintColor(.green3)
        ...
        let titleString = NSAttributedString(string: "Dashboard", attributes: [
        ...
        DashboardTitle.attributedText = attributedTitle
        DashboardTitle.translatesAutoresizingMaskIntoConstraints = false
        view.addSubview(DashboardTitle)
        
        NSLayoutConstraint.activate([
            DashboardTitle.leadingAnchor.constraint(equalTo: view.leadingAnchor, constant: 20),
           DashboardTitle.topAnchor.constraint(equalTo: view.safeAreaLayoutGuide.topAnchor, constant: 20)
        ])
    }
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        performSegue(withIdentifier: "ShowDashboardDetail", sender: self)
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "ShowDashboardDetail" {
            if let destinationVC = segue.destination as? DashboardDetail,
               let indexPath = tableView.indexPathForSelectedRow {
                let selectedProjects = projectsList[indexPath.row]
                destinationVC.dashboard = selectedProjects
            }
        }
    }
